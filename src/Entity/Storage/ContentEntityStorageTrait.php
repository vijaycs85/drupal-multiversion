...
trait ContentEntityStorageTrait {
  ...
  protected function buildQuery($ids, $revision_id = FALSE) {
    ...
    if (!is_subclass_of($this->entityType->getStorageClass(), 'Drupal\multiversion\Entity\Storage\ContentEntityStorageInterface')) {
      return $query;
    }
  ...
  protected function buildRevisionBranch(EntityInterface $entity) {
    ...
    if ($id = $entity->id()) {
      $latest_revision = $this->load($id);
      if ($latest_revision) {
        $max_id = $latest_revision->getRevisionId();
      }
    }
    if ($count_revisions > $i && isset($max_id)) {
      $i = $count_revisions > $max_id ? $count_revisions : $max_id;
    }
